...
import pickle
import importlib
item = importlib.import_module(f"dataset.ablation.numberckpt_200.finetune")
...
cache_file = os.path.join(os.path.dirname(checkpoint_path), f"performance_{tag}.cache") if cache_file is None else cache_file
...
log_file = os.path.join(os.path.dirname(cache_file), f"log_{tag}.txt")
